...
//$getTripDet=$client->GetTripDetailsV2("javaapitest","testing","71","100","11/02/2014","717ZPXF1OYG17N9"); 
$getTripDet=$client->GetTripDetailsV2("javaapitest","testing","71","100","11/02/2014","7BA1YI13XC85135");
...
<?php 	
foreach ($resTripDet as $bus=>$value) { 
print "<pre>";
//print_r($bus);
...
$resultBoardList =$resTripDet->boardingPointList;

foreach ( $resultBoardList as $board ) {  
$boardingId = $board->boardingPointId; 
$boardingName = $board->boardingPointName; 
$boardtime = $board -> boardtime;
}


$resultBusList =$resTripDet->busLayoutList;
foreach ( $resultBusList as $bus ) {  
	$detList = $bus -> seatDetailsList;
		foreach ( $detList as $resutl ) {  
			$cellType = $resutl->cellType; 
			$colnum = $resutl->columnNbr; 
			$rownum = $resutl->rowNbr; 
			$seatnum = $resutl->seatNbr;
		}
}

$resultDetList =$resTripDet->seatDetailList;
foreach ( $resultDetList as $seat ) {  
$fare = $seat->fare; 
$seatNum = $seat->seatNbr; 
