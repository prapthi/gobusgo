...
$getIdList=$client->GetFromToStationIdList("javaapitest","testing"); 
$ResGetIdList=$getIdList->fromToStationList; 

$outterArray = ((array)$getIdList);
$innerArray = ((array)$outterArray['fromToStationList']);


foreach($innerArray as $holdingObject){
	$holdingArray = ((array)$holdingObject);
	$phoneNum = $holdingArray['fromStationId'];
	foreach($holdingArray as $to){
...
<?php 
	foreach ( $ResGetIdList as $fromList ) {  
		$fromId = $fromList->fromStationId;
		$toId = $fromList->toStationId;
		print "<pre>";
		print_r($fromList);
		print "</pre>";
		}
...
<select id="fromid" class="bus_input" title="from" name="from">
	...
	?> 
	<option  value="<?php echo $fromId; ?>"><?php echo $fromId; ?></option> 
